...
class Neuron():
    ...
    def __init__(self, n_in: int, w: np.array = None, b: float = 0, non_lin: bool = True) -> None:
        assert isinstance(
            n_in, int) and n_in > 0, 'Number of inputs must be a positive non-zero integer'
...
class NeuralLayer():
    ...
    def __init__(self, n_in: int, n_units: int, W: np.array = None, b: np.array = None, act=None) -> None:
        assert isinstance(
            n_in, int) and n_in > 0, 'Number of inputs must be a positive non-zero integer'
        assert isinstance(
            n_units, int) and n_units > 0, 'Number of neurons in a layer must be a positive non-zero integer'
        if W is not None:
            assert W.shape == (
                n_in, n_units), f"Invalid shape for layer weight's matrix W: {W.shape}, expected {(n_in, n_units)}"
        if b is not None:
            assert b.shape == (
                n_units,), f"Invalid shape for layer bias b: {b.shape}, expected {(n_units,)}"
